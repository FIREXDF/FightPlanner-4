// Mod Operations
// Gère les opérations sur les mods (rename, toggle, uninstall)
...
class ModOperations {
    ...
    async renameMod(mod) {
        if (!mod.folderPath) {
            if (window.toastManager) {
                window.toastManager.error('Cannot rename this mod - folder path not found');
            }
            return;
        }
        ...
        if (window.modalManager) {
            window.modalManager.openRenameModal(mod, async (newName) => {
                if (window.electronAPI && window.electronAPI.renameMod) {
                    const result = await window.electronAPI.renameMod(mod.folderPath, newName);
                    ...
                    if (result.success) {
                        ...
                        if (window.toastManager) {
                            window.toastManager.error(`Failed to rename mod: ${result.error}`);
                        }
                    }
                }
            });
    ...
    async toggleModStatus(mod) {
        ...
        if (window.electronAPI && window.electronAPI.toggleMod) {
            const result = await window.electronAPI.toggleMod(mod.folderPath, this.modManager.modsPath);
            
            if (result.success) {
                console.log(`Mod ${result.isNowActive ? 'enabled' : 'disabled'} successfully`);
                if (window.toastManager) {
                    window.toastManager.success(`Mod ${result.isNowActive ? 'enabled' : 'disabled'}`);
                }
                // Refresh mod list
                this.modManager.fetchMods();
            } else {
    ...
    async uninstallMod(mod) {
        if (!mod.folderPath) {
            if (window.toastManager) {
                window.toastManager.error('Cannot uninstall this mod - folder path not found');
            }
            return;
        }
            ...
            window.modalManager.openUninstallModal(mod, async () => {
                if (window.electronAPI && window.electronAPI.deleteMod) {
                    ...
                    if (result.success) {
                        ...
                        // Clear selection if this was the selected mod
                        if (this.modManager.selectedMod && this.modManager.selectedMod.id === mod.id) {
                        ...
                        if (window.toastManager) {
                            window.toastManager.success(`Mod "${mod.name}" has been uninstalled`);
                        }
                        ...
                        if (window.toastManager) {
                            window.toastManager.error(`Failed to uninstall mod: ${result.error}`);
                        }
                    }
                }
            });
    ...
    async changeSlot(mod) {
        ...
        if (window.electronAPI && window.electronAPI.scanModSlots) {
            ...
            if (result.success && result.slots) {
                if (window.modalManager) {
                    window.modalManager.openChangeSlotModal(mod, result.slots, async (changes) => {
                        if (window.electronAPI && window.electronAPI.applySlotChanges) {
                            const applyResult = await window.electronAPI.applySlotChanges(mod.folderPath, changes);
                            
                            if (applyResult.success) {
                                if (window.toastManager) {
                                    window.toastManager.success('Slot changes applied successfully');
                                }
                                // Refresh mod list
                                this.modManager.fetchMods();
                            } else {
                                if (window.toastManager) {
                                    window.toastManager.error(`Failed to apply slot changes: ${applyResult.error}`);
                                }
                            }
                        }
                    });
                ...
                if (window.toastManager) {
                    window.toastManager.error(`Failed to scan mod slots: ${result.error || 'Unknown error'}`);
                }
            }
        }
    }
