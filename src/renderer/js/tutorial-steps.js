// Tutorial Steps Manager for transparent window
...
// Initialize tutorial with migration check
    ...
    if (window.tutorialAPI && window.tutorialAPI.getMigrationStatus) {
        try {
            ...
            if (migrationStatus.success && migrationStatus.completed) {
                ...
                // Add migration step after welcome
                const migrationStep = {
                ...
                // Insert migration step after welcome (index 1)
...
// Start animation sequence with Lottie
function startAnimation() {
    ...
    // Step 1: Background devient blanc à 2.33 secondes (quand l'animation l'indique)
    setTimeout(() => {
        tutorialWindow.classList.add('white-bg');
    }, 2330 + 200); // 2.33s + 200ms delay

    // Step 2: Animation Lottie terminée, on la cache (3s + 200ms)
    setTimeout(() => {
        lottieContainer.style.opacity = '0';
    }, 3200);

    // Step 3: Show welcome text (3500ms)
    setTimeout(() => {
        welcomeText.classList.add('show');
    }, 3500);

    // Step 4: Screenshot apparaît EN BAS (flou) (4500ms)
    setTimeout(() => {
        screenshotPreview.classList.add('show');
    }, 4500);

    // Step 5: Welcome text + Screenshot slide vers le haut ENSEMBLE + DÉFLOUTAGE (5000ms)
    setTimeout(() => {
        welcomeText.classList.add('move-up');
        screenshotPreview.classList.add('slide-up');
        screenshotPreview.classList.add('clear'); // Défloute EN MÊME TEMPS qu'il monte
    }, 5000);

    // Step 6: Tout disparaît et tutorial container apparaît (8000ms)
    setTimeout(() => {
    ...
    // Step 7: Show tutorial container with steps (9000ms)
    setTimeout(() => {
...
function renderProgressDots() {
    ...
    container.innerHTML = steps.map((_, index) => `
        <div class="tutorial-progress-dot ${index === 0 ? 'active' : ''}" data-step="${index}"></div>
    `).join('');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize tutorial (checks migration and starts animation)
    setTimeout(() => {
        initializeTutorial();
    }, 100);
