// Tutorial Manager - Manages the onboarding tutorial
...
class TutorialManager {
    ...
    async initialize() {
        // Tutorial is now handled by main.js on first launch
        // This is just for manual opening from the app
        ...
        this.overlay.innerHTML = `
            ...
            <div class="tutorial-container" style="display: none;">
                ...
                <div class="tutorial-progress">
                    ${this.steps.map((_, i) => `
                        <div class="tutorial-progress-dot ${i === 0 ? 'active' : ''}" data-step="${i}"></div>
                    `).join('')}
                </div>
    ...
    playIntroAnimation() {
        ...
        const welcomeScreen = this.overlay.querySelector('.tutorial-welcome-screen');
        ...
        if (noAnimations) {
            // Skip all animations and show tutorial directly
        ...
        // Step 1: Show overlay (transparent background)
        setTimeout(() => {
            this.overlay.classList.add('show');
        }, 10);
        
        // Step 2: Blue bubble appears and starts floating around (0-3s)
        setTimeout(() => {
            bubble.classList.add('show');
        }, 200);
        
        // Step 3: After floating, bubble moves to center and expands to white (3s-4.2s)
        setTimeout(() => {
            bubble.classList.add('expand');
        }, 3200);
        
        // Step 4: Main app fades/blurs during expansion
        setTimeout(() => {
            if (mainContainer) {
        ...
        // Step 5: Welcome text appears on white background (4.5s)
        setTimeout(() => {
            welcomeScreen.classList.add('show');
        }, 4500);
        
        // Step 6: Wait 2 seconds, then transition to tutorial (6.5s)
        setTimeout(() => {
    ...
    async skip() {
        if (confirm('Are you sure you want to skip the tutorial? You can always restart it from the console.')) {
            this.complete();
        }
    }
    ...
    close() {
        if (this.overlay) {
            ...
            // Remove slide-up class from main container
            const mainContainer = document.querySelector('.main-container');
            if (mainContainer) {
    ...
    // Force show tutorial without restarting (for testing)
    forceShow() {
        this.show();
        console.log('✓ Tutorial window opened.');
    }
...
if (typeof window !== 'undefined') {
    ...
    console.log('   → window.tutorial.resetFirstLaunch() - Test first launch behavior');
}
